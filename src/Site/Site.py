...
class Site(object):
    ...
    def downloadContent(self, inner_path, download_files=True, peer=None, check_modifications=False, diffs={}):
        ...
        if download_files:
            for file_relative_path in self.content_manager.contents[inner_path].get("files", {}).keys():
                ...
                if not diff_success:
                    ...
                    res = self.needFile(file_inner_path, blocking=False, update=self.bad_files.get(file_inner_path),
                                        peer=peer)
        ...
        for file_relative_path in self.content_manager.contents[inner_path].get("includes", {}).keys():
            file_inner_path = content_inner_dir + file_relative_path
            include_thread = gevent.spawn(self.downloadContent, file_inner_path, download_files=download_files,
                                          peer=peer)
        ...
        while 1:
            ...
            if modified_contents:
                ...
                modified_contents.sort(
                    key=lambda inner_path: 0 - res["modified_files"][inner_path])  # Download newest first
    ...
    def checkModifications(self, since=None):
        ...
        if not queried:  # Start another 3 thread if first 3 is stuck
            peers_try[0:0] = [peer for peer in self.getConnectedPeers() if
                              peer.connection.connected]  # Add connected peers
            for _ in range(10):
    ...
    def publish(self, limit="default", inner_path="content.json", diffs={}, cb_progress=None):
        ...
        peers = sorted(peers, key=lambda peer: peer.connection.handshake.get("rev",
                                                                             0) < config.rev - 100)  # Prefer newer clients
        ...
        for i in range(min(len(peers), limit, threads)):
            publisher = gevent.spawn(self.publisher, inner_path, peers, published, limit, diffs, event_done,
                                     cb_progress)
        ...
        for content_inner_path, content in self.content_manager.contents.items():
            ...
            for file_relative_path in file_relative_paths:
                ...
                if not file_inner_path.startswith(root_inner_path):
                    ...
                    continue
                if file_inner_path.split("/")[
                    0] in default_dirs:  # Dont copy directories that has -default postfixed alternative
                ...
                if file_inner_path_dest.replace("-default",
                                                "") == "content.json":  # Don't copy root content.json-default
                    continue
    ...
    def announcePex(self, query_num=2, need_num=5):
        peers = [peer for peer in self.peers.values() if
                 peer.connection and peer.connection.connected]  # Connected peers
    ...
    def announceTracker(self, tracker_protocol, tracker_address, fileserver_port=0, add_types=[], my_peer_id="",
                        mode="start"):
        s = time.time()
    ...
    def announce(self, force=False, mode="start", pex=True):
        ...
        if trackers and (
                        mode == "update" or mode == "more"):  # Only announce on one tracker, increment the queried tracker id
    ...
    def getConnectablePeers(self, need_num=5, ignore=[]):
        ...
        if need_num > 5 and need_num < 100 and len(found) < need_num:  # Return not that good peers
            found = [peer for peer in peers if not peer.key.endswith(":0") and peer.key not in ignore][
    ...
    def getConnectedPeers(self):
        ...
        for connection in self.connection_server.connections:
            if not connection.connected and time.time() - connection.start_time > 20:  # Still not connected after 20s
                continue
            # print("Site getConnectedPeers:" + str(self.peers))
    ...
    def cleanupPeers(self, peers_protected=[]):
        ...
        closed = 0
        connected_peers = [peer for peer in self.getConnectedPeers() if
                           peer.connection.connected]  # Only fully connected peers
        ...
        if need_to_close > 0:
            pass
            # self.log.debug(
            #     "Connected: %s, Need to close: %s, Closed: %s" % (len(connected_peers), need_to_close, closed))
    ...
    def updateHashfield(self, limit=5):
        ...
        if not self.content_manager.hashfield and not self.content_manager.contents.get("content.json", {}).get(
                "files_optional"):
            return False
    ...
    def addEventListeners(self):
        ...
        self.onFileStart.append(
            lambda inner_path: self.fileStarted())  # No parameters to make Noparallel batching working
