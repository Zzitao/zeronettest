...
class UiWebsocket(object):
    ...
    def addHomepageNotifications(self):
        ...
        if file_server.port_opened is True:
            self.site.notifications.append([
                "done",
                _["Congratulation, your port <b>{0}</b> is opened.<br>You are full member of ZeroNet network!"].format(
    ...
    def cbSitePublish(self, to, site, thread, notification=True, callback=True):
        ...
        if published > 0:  # Successfully published
            ...
            if len(site.peers) == 0:
                if sys.modules["main"].file_server.port_opened or sys.modules[
                    "main"].file_server.tor_manager.start_onions:
                    if notification:
                    ...
                    if notification:
                        self.cmd("notification", [
                            ...
                            {_[on your router to make your site accessible for everyone.]}""").format(
                                config.fileserver_port)
                        ])
                    if callback:
    ...
    def actionFileWrite(self, to, inner_path, content_base64, ignore_bad_files=False):
        ...
        content_inner_path = re.sub("^(.*)/.*?$", "\\1/content.json",
                                    inner_path)  # Also check the content.json from same directory
        if (self.site.bad_files.get(inner_path) or self.site.bad_files.get(
                content_inner_path)) and not ignore_bad_files:
            found = self.site.needFile(inner_path, update=True, priority=10)
            if not found:
                self.cmd(
                    "confirm",
                    [_["This file still in sync, if you write it now, then the previous content may be lost."],
        ...
        if inner_path.endswith("content.json") and rules:
            ...
            if content:
                rules["current_size"] = len(json.dumps(content)) + sum(
                    [file["size"] for file in content["files"].values()])
            else:
    ...
    def actionCertAdd(self, to, domain, auth_type, auth_user_name, cert):
        try:
            res = self.user.addCert(self.user.getAuthAddress(self.site.address), domain, auth_type, auth_user_name,
                                    cert)
            if res is True:
                self.cmd(
                    "notification",
                ...
                body = _(
                    "{_[Your current certificate]:} <b>{cert_current[auth_type]}/{cert_current[auth_user_name]}@{domain}</b>")
                self.cmd(
                    "confirm",
    ...
    def actionCertSelect(self, to, accepted_domains=[], accept_any=False):
        ...
        more_domains = [domain for domain in accepted_domains if
                        domain not in self.user.certs]  # Domains we not displayed yet
        if more_domains:
    ...
    def actionSiteClone(self, to, address, root_inner_path="", target_address=None):
        ...
        if target_address:
            ...
            self.cmd("notification",
                     ["done", _["Site cloned"] + "<script>window.top.location = '/%s'</script>" % new_address])
