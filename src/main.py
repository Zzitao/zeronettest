...
class Actions(object):
    def call(self, function_name, kwargs):
        logging.info(
            "Version: %s r%s, Python %s, Gevent: %s" % (config.version, config.rev, sys.version, gevent.__version__))
        logging.info(
    ...
    def sitePublish(self, address, peer_ip=None, peer_port=15441, inner_path="content.json", diffs={}):
        ...
        #     logging.info(
        #         my_peer.request("sitePublish", {"site": site.address, "inner_path": inner_path, "diffs": diffs}))
        #     logging.info("Done.")
