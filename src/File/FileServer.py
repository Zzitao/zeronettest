...
class FileServer(ConnectionServer):
    def __init__(self, ip=config.fileserver_ip, port=config.fileserver_port):
        ...
        # self.log.info("fileserver_ip : %s fileserver_port : %s " % (str(ip), str(port)))
        # print("fileserver_ip : %s fileserver_port : %s " % (str(ip), str(port)))
        ConnectionServer.__init__(self, ip, port, self.handleRequest)
        # self.log.info("ip_external : %s" % str(config.ip_external))
        # print("ip_external : %s" % str(config.ip_external))
    ...
    def handleRequest(self, connection, message):
        if config.verbose:
            if "params" in message:
                ...
                #     "FileRequest: %s %s %s %s" %
                #     (
                #         str(connection), message["cmd"], message["params"].get("site"),
    ...
    def testOpenportPortchecker(self, port=None):
        ...
        try:
            ...
            message = re.sub("<.*?>", "",
                             message.replace("<br>", " ").replace("&nbsp;", " ").strip())  # Strip http tags
    ...
    def checkSites(self, check_files=False, force_port_check=False):
        ...
        if not sites_checking:
            for site in sorted(self.sites.values(), key=lambda site: site.settings.get("modified", 0),
                               reverse=True):  # Check sites integrity
                check_thread = gevent.spawn(self.checkSite, site, check_files)  # Check in new thread
                time.sleep(2)
                if site.settings.get("modified",
                                     0) < time.time() - 60 * 60 * 24:  # Not so active site, wait some sec to finish
    ...
    def cleanupSites(self):
        ...
        while 1:
            ...
            # self.log.debug("Running site cleanup, connections: %s, internet: %s, protected peers: %s" % (
            #     len(self.connections), self.has_internet, peers_protected))
            ...
            for address, site in self.sites.items():
                ...
                if not startup:  # Don't do it at start up because checkSite already has needConnections at start up.
                    connected_num = site.needConnections(
                        check_site_on_reconnect=True)  # Keep active peer connection to get the updates
    ...
    def announceSites(self):
        ...
        #         if site.settings[
        #             "own"] or active_site:  # Check connections more frequently on own and active sites to speed-up first connections
        ...
        #     taken = time.time() - s
        #
        #     sleep = max(0, 60 * 20 / len(
        #         config.trackers) - taken)  # Query all trackers one-by-one in 20 minutes evenly distributed
